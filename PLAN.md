...
Source documents: `CLAUDE.md`, `docs/ARCHITECTURE_EN.md`. If conflicts arise, `ARCHITECTURE_EN.md` wins.
...
- Create empty package directories matching `ARCHITECTURE_EN.md` §16: `ui/screen`, `ui/component`, `presentation`, `audio`, `imaging`, `domain/{model,nlu,state,safety,usecase}`, `data/{protocol,visual,local}`, `ai/{melange,orchestrator,prompt,model}`, `assets/{protocols,visuals/images,demo_inputs,knowledge}`.
...
- [ ] Package directory layout matches `ARCHITECTURE_EN.md` §16 1:1.
...
- `UserTurn` (`ARCHITECTURE_EN.md` §11.1)
...
### Checkpoints
- [ ] JSON schema matches `ARCHITECTURE_EN.md` §12 exactly (`tree_id`, `start_node`, `nodes[].transitions`, etc.).
...
Enter only after S9 is stable. Follows `ARCHITECTURE_EN.md` §10.2 Profile B / §24.1 ordering.
...
- **Doc updates:** At the end of S3, S7, and S8, sync any divergence back into `ARCHITECTURE_EN.md` (per CLAUDE.md).
